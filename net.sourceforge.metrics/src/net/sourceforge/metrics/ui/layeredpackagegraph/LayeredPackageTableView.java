...
/**
 * View that renders the metrica in a table and reacts to selection events and
 * resource change events from the environment
 * 
 * @author Frank Sauer
 */
public class LayeredPackageTableView extends ViewPart implements
        ISelectionListener, IMetricsProgressListener,
     ...
     * The constructor.
     */
    public LayeredPackageTableView() {
    }
    ...
    public void createPartControl(Composite parent) {
        ...
        mActions = new LayeredPackageGraphActionGroup(this);
    ...
    private void createTable(Composite c) {
        table = new LayeredPackageTable(c, SWT.FULL_SELECTION);
    ...
    private void refreshTable(final AbstractMetricSource ms,
    ...
    public void selectionChanged(IWorkbenchPart part, ISelection selection) {
        if (selection instanceof IStructuredSelection) {
            ...
            if (first instanceof IJavaElement) {
                ...
                if (canDoMetrics(jElem)) setJavaElement(jElem, false);
            }
        }
    }
    ...
    /**
     * display dependency graph as embedded workbench view on Windows, in a
     * separate AWT frame on all other platforms. As of 4/30/04, only 3.0M8+ and
     * embedded style on all platforms
     */
    public void displayDependencyGraph() {
        /*
         * if (selection.getElementType() <= IJavaElement.PACKAGE_FRAGMENT_ROOT) {
         * IGraphContributor source = (IGraphContributor)
         * Dispatcher.getAbstractMetricSource(selection); if
         * ("win32".equals(BootLoader.getWS())) {
         * displayDependencyGraphSWT(source.getEfferent()); } else {
         * displayDependencyGraphAWT(source.getEfferent()); } }
         */// now works the same on all platforms
        IGraphContributor source = (IGraphContributor) Dispatcher
                .getAbstractMetricSource(selection);
    ...
    private void displayDependencyGraphSWT(final Map graph) {
        ...
        if (page != null) {
            try {
                page.showView(LayeredPackageGraphView.class.getName());
    ...
    /*
     * private void displayDependencyGraphAWT(final Map graph) { Thread awt =
     * new Thread(new Runnable() { public void run() { final Frame frame = new
     * Frame("Dependencies"); final DependencyGraphPanel glPanel = new
     * DependencyGraphPanel(); try { glPanel.createDependencies(graph); } catch
     * (TGException e1) { Log.logError("Could not create DependencyGraphPanel",
     * e1); } frame.addWindowListener(new WindowAdapter() { public void
     * windowClosing(WindowEvent e) { frame.remove(glPanel); frame.dispose(); }
     * }); frame.add("Center", glPanel); frame.setSize(800,600);
     * frame.setVisible(true); } }); awt.start(); }
     */
    ...
    public void exportXML() {
        if (selection != null) {
            ...
            if (fileName != null) {
                ...
                IExporter exporter = MetricsPlugin.getDefault()
                        .getCurrentExporter();
                if (exporter != null) doExport(activeShell, outputFile,
                        exporter);
                else {
    ...
    private void doExport(Shell activeShell, final File outputFile,
    ...
    public void pending(IJavaElement current) {
        setStatus("Queued: " + queued + "\tCalculating now: "
                + current.getElementName(), shouldBeBusy(current));
    }
    ...
    private boolean shouldBeBusy(IJavaElement current) {
        ...
        busy = busy
                || (selection != null)
                && (current.getHandleIdentifier().startsWith(selection
                        .getHandleIdentifier()));
        return busy;
    }
    ...
    public void completed(IJavaElement element, Object data) {
        setStatus("completed " + element.getElementName(),
                shouldBeBusy(element));
    ...
    public void projectComplete(IJavaProject project, boolean aborted) {
        ...
        boolean showProject = MetricsPlugin.getDefault()
                .showProjectOnCompletion();
     ...
     * (non-Javadoc)
     * 
     * @see net.sourceforge.metrics.core.MetricsBuilder.MetricsProgressListener#moved(java.lang.String,
    ...
    public static int getLayer(String depPackageName) {
        return LayeredPackageTable.getLayer(depPackageName);
    }
