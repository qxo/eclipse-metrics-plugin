...
public class LayeredPackageGraphPanel extends GLPanel {
    ...
    public void createDependencies(List layers) throws TGException {
        ...
        StrongComponent[] components = calculateCycles(LayeredPackageTableView
                .getDependencies());
        int layer = 0;
        for (Iterator i = layers.iterator(); i.hasNext(); layer++) {
            ...
            for (Iterator d = packageStats.iterator(); d.hasNext();) {
                ...
                Node from = addNode(stats.getPackageName(), stats.getLayer(),
                        ++layerWidth[layer]);
                if (stats.getLayer() > max) {
                    max = stats.getLayer();
                    center = from;
                }
                Set deps = (Set) LayeredPackageTableView.getDependencies().get(
                        stats.getPackageName());
                for (Iterator d2 = deps.iterator(); d2.hasNext();) {
                    String depPackageName = (String) d2.next();
                    int depLayer = LayeredPackageTableView
                            .getLayer(depPackageName);
                    Node to = addNode(depPackageName, depLayer,
                            ++layerWidth[depLayer]);
    ...
    /**
     * Calculate strongly connected components in dependency graph, basically
     * adaper code to map different graph representations
     * 
     * @param efferent
     * @return
     */
    ...
    /**
     * cluster strongly connected packages (cycles) together, closer than
     * packages not part of the strong connection.
     * 
     * @param to
     * @param from
     */
     ...
     * @param knots
     */
    private void addKnot(Node from, Node to, StrongComponent component,
            int index) {
        try {
            ...
            if (knot == null) {
                ...
                if (length > 1) {
                    addKnotToMenu(from.getLabel()[0] + " et al. (" + lbl + ")",
                            knot);
                }
            }
    ...
    /**
     * Adds a node for the given id, unless it already exists. The node's label
     * will be the id, unless it starts with an "=" in which case it is treated
     * as a IJavaElement handle and the elements name will be used as the label.
     * 
     * @param id
     * @return
     * @throws TGException
     */
    private Node addNode(String id, int layer, int layerWidth)
            throws TGException {
    ...
    private Color getGraphBackground() {
        RGB color = PreferenceConverter.getColor(MetricsPlugin.getDefault()
                .getPreferenceStore(), "METRICS.depGR_background");
