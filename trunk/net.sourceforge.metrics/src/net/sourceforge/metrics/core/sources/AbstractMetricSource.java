...
/**
 * Base class for all metric sources. Used by calculators to retrieve the
 * AST, IJavaElement, etc. and to store their results.
 * 
 * @author Frank Sauer
 */
public abstract class AbstractMetricSource implements Constants, Serializable {
	...
	public AbstractMetricSource[] getChildrenHaving(String per, String metric) {
		...
		for (Iterator i = childHandles.iterator(); i.hasNext();) {
			IJavaElement elm = JavaCore.create((String)i.next());
			if (elm != null) {
		...
		return (AbstractMetricSource[]) result.toArray(new AbstractMetricSource[]{});
	}
	 ...
	 * As a side effect, adds a marker of type net.sourceforge.metrics.outofrangemarker
	 * if enabled and the value is out of the range indicated by the metric's descriptor
	 * @param value
	 */
	...
	private void checkRange(Metric value) {
		...
		if (!md.isValueInRange(value.doubleValue())) {
			try {
				...
				if (resource != null) configureOutOfRangeMarker(value, md, resource);
			} catch (Throwable e) {
	...
	public void setAverage(Avg value) {
		//System.err.println("AVG " + input.getElementName() + "."+value.getName() + " per " + value.getPer() + " = " + value.doubleValue());
	...
	public void setMaximum(Max value) {
		//System.err.println("MAX " + input.getElementName() + "."+value.getName() + " per " + value.getPer() + " = " + value.doubleValue());
	 ...
	 * Used to prematurely abort a long calculation
	 * @return boolean
	 */
	...
	/**
	 * Get a list of all metrics defined in this node's children with the
	 * given name
	 * @param name
	 * @return List
	 */	
	public List getMetricsFromChildren(String name) {
		List metrics = new ArrayList();
		for (Iterator i = getChildren().iterator(); i.hasNext();) {
			AbstractMetricSource next = (AbstractMetricSource)i.next();
			Metric m = (Metric)next.getValue(name);
	...
	/**
	 * calculate average of metrics defined in this node's children with the
	 * given name
	 * @param name
	 * @param per
	 * @return List
	 */	
	public List getAveragesFromChildren(String name, String per) {
	...
	/**
	 * calculate maximum of metrics defined in this node's children with the
	 * given name
	 * @param name
	 * @param per
	 * @return List
	...
	public Avg getAverage(String name, String per) {
		return (Avg)averages.get(per+name);
	}
	...
	public Max getMaximum(String name, String per) {
		return (Max)maxima.get(per+name);
	}
	...
	/**
	 * Get the AST of this node. Used frequently by calculators. This
	 * base class returns null. Implemented by CompilationUnitMetrics
	 * @return IJavaElement
	...
	/**
	 * get the original compilation unit for the given IJavaElement.
	 * If the compilation unit turns out to be a WorkingCopy this method
	 * returns its original source compilation unit
	 * @param input
	 * @return ICompilationUnit
	 */
	...
	/**
	 * Sets the doRecurse.
	 * @param doRecurse The doRecurse to set
	 */
